d3.select(window).on("load", function () {
    ...
    data.forEach((item, item_no) => {
        ...
        var row = d3
            ...
            .attr("class", "form-row");
        var col = row.append("div").attr("class", "col-12");

        if ("label" in item) {
            col.append("span").html(item["label"]);
        } else if ("question" in item) {
            var div = col.append("div").attr("class", "form-group");
            ...
            if ("choices" in item) {
                ...
                item["choices"].forEach((choice, i) => {
                    ...
                    var html = `<input class="form-check-input item${item_no}-radios form-control" 
                                     type="radio" 
                                     name="item${item_no}-radios" 
                                     id="item${item_no}-radio${i}" 
                                     value="option${i}" 
                ...
                div
                    ...
                    .attr("class", "invalid-feedback")
                    .html(
                        `Please provide an answer, using at least ${min_text_length} characters.`
                    );
            }
        }
    });
    ...
    d3.selectAll("textarea").on("keyup", function (d) {
    ...
    d3.select("#submit-btn").on("click", function () {
        ...
        data.forEach((item, item_no) => {
            if ("label" in item) {
                ...
                if ("choices" in item) {
                    // multiple choice question
                    var selected = d3.select(
                        `input[name="item${item_no}-radios"]:checked`
                    );
                    ...
                    if (text.length < min_text_length) {
                        ...
                        errors = true;
                        d3.select(`item${item_no}-feedback`).style("display", "block");
                    } else {
                        d3.select(`.item${item_no}-feedback`).style("display", "hidden");
                    }
                }
            }
            // d3.selectAll('form')
